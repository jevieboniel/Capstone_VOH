  ...
  try {
    ...
    try {
      await logAudit(req, {
        ...
        resource: "Alert",
        resourceId: alertId,
        details: `Created alert "${title}" (${status}) to roles: ${roles.join(", ")}`,
        severity: safePriority === "high" ? "warning" : "info",
      });
    } catch (e) {
      console.error("Audit log failed (CREATE alert):", e);
    }
  ...
  try {
    ...
    try {
      await logAudit(req, {
        ...
        resource: "Alert",
        resourceId: alertId,
        details: "Resent failed recipients",
        severity: "info",
      });
    } catch (e) {
      console.error("Audit log failed (RESEND failed recipients):", e);
    }
