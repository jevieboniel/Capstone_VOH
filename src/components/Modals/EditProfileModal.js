...
const shouldClearReintegration = (status, adoptionStatus) => {
  // ✅ any combo other than Reintegrated + Adopted => clear reintegration
  return !isReintegratedPair(status, adoptionStatus);
};
...
const EditProfileModal = ({ child, onClose, onSave }) => {
  ...
  // ✅ detect if this child has reintegration in DB
  const hasExistingReintegration = useMemo(() => {
    return !!(child?.reintegration || child?.reintegration_details);
  }, [child]);

  // ✅ show warning banner when user is about to clear reintegration
  const [showReintegrationClearWarning, setShowReintegrationClearWarning] = useState(false);
  ...
  useEffect(() => {
    ...
    setPreview(null);
    setShowReintegrationClearWarning(false);
  }, [child]);
  ...
  const update = (key, value) => {
    setForm((p) => {
      ...
      // ✅ When status/adoption changes, decide if reintegration will be cleared
      const status = key === "status" ? value : next.status;
      const adoptionStatus = key === "adoptionStatus" ? value : next.adoptionStatus;

      const willClear = hasExistingReintegration && shouldClearReintegration(status, adoptionStatus);

      setShowReintegrationClearWarning(willClear);
  ...
  const handleSave = () => {
    // ✅ If user sets it back to Active/Open/etc, automatically remove reintegration (frontend)
    const next = { ...child, ...form };

    if (shouldClearReintegration(next.status, next.adoptionStatus)) {
      next.reintegration = null; // ✅ clear in payload
    }
  ...
  return (
    <ModalShell title="Edit Child" subtitle={fullName} onClose={onClose} maxWidth="max-w-3xl">
      ...
      {/* ✅ warning banner */}
      {showReintegrationClearWarning ? (
        <div className="mb-5 p-4 rounded-2xl border border-amber-200 dark:border-amber-900/40 bg-amber-50 dark:bg-amber-950/20 text-amber-900 dark:text-amber-200">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              <p className="font-semibold">Reintegration details will be cleared</p>
              <p className="text-sm mt-1 opacity-90">
                This child has existing reintegration data. Changing Status/Adoption Status away from
                <span className="font-medium"> Reintegrated + Adopted</span> will remove those details.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Input
            ...
            onChange={(e) => update("lastCheckup", e.target.value)}
        />
        </Field>
        ...
        <div className="md:col-span-3">
          <Field label="Profile Image / Photo">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">Leave empty if you don’t want to change the photo.</p>
          </Field>
        </div>
