    ...
    return {
        ...
        childId: Number(
        m.childId ?? m.child_id ?? m.childID ?? m.child ?? 0
        ),
    ...
    useEffect(() => {
        ...
        try {
            ...
            // API can return array or { milestones: [] }
            const raw = Array.isArray(data) ? data : data?.milestones || [];

            if (!mounted) return;

            // ✅ Normalize so stats match Development.js
            const normalized = raw
            .map(normalizeMilestone)
            // extra safety: ensure this modal only uses THIS child's milestones
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-3 md:p-4">
        <div className="w-full max-w-4xl max-h-[85vh] bg-white rounded-2xl shadow-xl overflow-hidden flex flex-col">
            {/* Header */}
            <div className="px-6 py-5 flex items-start justify-between">
            <div className="min-w-0">
                <h2 className="text-2xl font-bold text-gray-900">Development Tracking</h2>
            ...
            <div className="rounded-2xl bg-blue-50 border border-blue-100 p-6">
                ...
                <p className="text-lg font-medium text-gray-900">Overall Progress</p>
                <p className="text-3xl font-bold text-blue-700">{stats.overall}%</p>
                </div>
                <div className="mt-3">
                <ProgressBar value={stats.overall} />
                </div>
                <div className="mt-4 flex flex-wrap gap-6 text-sm">
                <span className="text-gray-700">{stats.total} milestones total</span>
                <span className="text-green-700">{stats.completed} completed</span>
                <span className="text-blue-700">{stats.inProgress} in progress</span>
                </div>
            </div>

            {/* Areas */}
            <div className="rounded-2xl border bg-white p-6">
                <p className="text-lg font-medium text-gray-900 mb-4">Development Areas</p>
                ...
                {[
                    ...
                    <div className="flex items-center justify-between mb-2">
                        ...
                        <p className="text-sm font-medium text-gray-900">{x.value}%</p>
                    </div>
            ...
            <div className="space-y-4">
                <p className="text-lg font-medium text-gray-900">Active Milestones</p>

                {loading && <div className="text-sm text-gray-500">Loading milestones…</div>}
                ...
                <div key={m.id} className="rounded-2xl border bg-white p-5">
                    ...
                    {m.notes && <p className="mt-3 text-sm text-gray-700">{m.notes}</p>}
                </div>
                ))}
