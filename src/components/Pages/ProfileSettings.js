...
const CONTROL = `${CONTROL_H} w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 text-sm text-gray-900 dark:text-gray-100 placeholder:text-gray-400 outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500`;
...
// ✅ FIX: make avatar URLs absolute so /uploads/... loads from backend
const API_ORIGIN = "http://localhost:5000";
const toAbsoluteAvatarUrl = (url) => {
  if (!url) return "";
  if (url.startsWith("blob:") || url.startsWith("data:")) return url;
  if (url.startsWith("http://") || url.startsWith("https://")) return url;
  if (url.startsWith("/")) return `${API_ORIGIN}${url}`; // ✅ /uploads/... -> http://localhost:5000/uploads/...
  return url;
};
...
const ProfileSettings = ({
  ...
  const [avatarPreview, setAvatarPreview] = useState(
    () => toAbsoluteAvatarUrl(initialUser.avatarUrl || "")
  );

  // ✅ eslint exhaustive-deps fix (fixed)
  useEffect(() => {
    setForm(initialUser);
    setAvatarPreview(toAbsoluteAvatarUrl(initialUser.avatarUrl || ""));
  }, [initialUser]);
      ...
      <div className="mx-auto w-full max-w-[1200px] p-4 sm:p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          ...
          <div className="lg:col-span-1 space-y-6">
            <Card
              title="Profile Photo"
              description="This will be shown on your account."
            >
            ...
            <Card title="Account" description="Your account details.">
              <div>
                ...
                <input
                  ...
                  className={`${CONTROL} ${
                    roleReadOnly ? "opacity-80 cursor-not-allowed" : ""
                  }`}
                />
                {roleReadOnly ? (
                  <p className={HELP}>Role is managed by the admin.</p>
                ) : null}
              </div>
            </Card>
          </div>
